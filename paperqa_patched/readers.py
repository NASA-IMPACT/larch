...
def read_doc_patched(
    ...
    Add text prepro
    """
    text_preprocessor = text_preprocessor or (lambda x: x)
    texts = read_doc(path, doc, chunk_chars, overlap, force_pypdf)
    for text in texts:
        text.text = text_preprocessor(text.text)
    return texts
