...
class Docs(BaseModel, arbitrary_types_allowed=True, smart_union=True):
    ...
    text_preprocessor: Callable = lambda x: x

    @property
    def index_path(self) -> Path:
        return PAPERQA_DIR / self.name if self._index_path is None else self._index_path
    ...
    def add(
        ...
        if citation is None:
            ...
            citation = f"{citation}[{os.path.basename(path)}]"
